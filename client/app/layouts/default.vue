<template>
  <div class="flex h-screen bg-[var(--ui-bg)]">
    <!-- Sidebar -->
    <aside
      class="border-r border-[var(--ui-border)]/50 flex flex-col bg-[var(--ui-bg-elevated)] shrink-0 transition-[width,opacity] duration-300"
      :class="isSidebarOpen ? 'w-64 opacity-100' : 'w-0 opacity-0 overflow-hidden border-none'"
    >
      <!-- Brand -->
      <div class="px-4 py-4 border-b border-[var(--ui-border)]/50 flex items-center justify-between">
        <NuxtLink to="/" class="flex items-center gap-2.5">
          <span class="w-4 h-0.5 bg-[var(--ui-primary)] rounded-full" />
          <div class="min-w-0">
            <h1 class="text-[14px] font-semibold text-[var(--ui-text-highlighted)] truncate">Research Council</h1>
            <p class="text-[11px] text-[var(--ui-text-dimmed)] leading-tight truncate">Decision Research</p>
          </div>
        </NuxtLink>
      </div>
      ...
      <div class="p-3">
        <UButton
          ...
          block
          size="sm"
          class="font-medium transition-transform active:scale-[0.98]"
        ...
        <div class="mb-4">
          ...
          <template v-if="activeSessions.length">
            <button
              ...
              :class="{ 'bg-[var(--ui-bg-accented)]': selectedSessionId === session.id }"
              @click="navigateToSession(session.id)"
            >
              <div class="flex items-center gap-2">
                <span class="w-1.5 h-1.5 rounded-full shrink-0" :class="phaseColor(session.phase)" />
              ...
              <p class="text-[11px] text-[var(--ui-text-dimmed)] mt-1 ml-3.5">{{ formatPhase(session.phase) }}</p>
            </button>
          </template>
          <p v-else class="text-[12px] text-[var(--ui-text-dimmed)] px-2 py-1">No active research</p>
        </div>

        <div class="h-px bg-[var(--ui-border)]/30 my-4 mx-2" />
          ...
          <template v-if="completedSessions.length">
            ...
            >
              ...
              <div class="flex items-center gap-2 mt-1">
                ...
                <span class="text-[11px] text-[var(--ui-text-dimmed)]">{{ session.confidenceLevel }}%</span>
              </div>
            </button>
          </template>
      ...
      <!-- Footer -->
      <div class="p-3 border-t border-[var(--ui-border)]/50 flex items-center justify-end bg-[var(--ui-bg)]">
        <ClientOnly>
          <UButton
    ...
    <main class="flex-1 overflow-hidden bg-[var(--ui-bg)] relative flex flex-col">
      <div v-if="showWorkspaceSidebarToggle" class="absolute top-[11px] left-4 z-50">
        <UButton
...
const showWorkspaceSidebarToggle = computed(() => !route.path.startsWith('/session/'))
